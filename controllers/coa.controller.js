module.exports = {
  async create(ctx) {
    try {
      ctx.body = await ctx.db.Coa.create({
  ...
  async findStatement(ctx) {
    try {
      ctx.body = await ctx.db.Coa.findAll({
        where: {
  ...
  async findOne(ctx) {
    try {
      ctx.body = await ctx.db.Coa.findByPk(ctx.params.id);
    } catch (err) {
      ctx.throw(500, err);
    }
  },
  async destroy(ctx) {
    try {
      var result = await ctx.db.Coa.destroy({
        where: {
          id: ctx.params.id,
        },
      });
  ...
  async update(ctx) {
    try {
      var result = await ctx.db.Coa.update(
        {
          description: ctx.request.body.description,
          ClaimantId: ctx.request.body.ClaimantId,
          JurisdictionId: ctx.request.body.JurisdictionId,
