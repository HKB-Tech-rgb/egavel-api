module.exports = {
  async create(ctx) {
    try {
      ctx.body = await ctx.db.LocusStandi.create({
  ...
  async findStatement(ctx) {
    try {
      ctx.body = await ctx.db.LocusStandi.findAll({
        where: {
  ...
  async findOne(ctx) {
    try {
      ctx.body = await ctx.db.LocusStandi.findByPk(ctx.params.id);
    } catch (err) {
      ctx.throw(500, err);
    }
  },
  async destroy(ctx) {
    try {
      var result = await ctx.db.LocusStandi.destroy({
        where: {
          id: ctx.params.id,
        },
      });
  ...
  async update(ctx) {
    try {
      var result = await ctx.db.LocusStandi.update(
        {
          description: ctx.request.body.description,
          ClaimantId: ctx.request.body.ClaimantId,
          totalUploads: ctx.request.body.totalUploads,
